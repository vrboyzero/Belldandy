...
// NOTE: 这些测试使用 node:sqlite，这是 Node.js 22+ 的实验性内置模块
// vitest/vite 目前无法正确解析 node:sqlite 模块
// 使用 describe.skip 暂时跳过这些测试，改用运行时环境测试或直接用 node 运行
// 当 vitest 支持 node:sqlite 后可以移除 .skip
