...
describe("file tools", () => {
    ...
    describe("file_read", () => {
        ...
        it("should truncate large files", async () => {
            ...
            const result = await fileReadTool.execute({ path: "large.txt", maxBytes: 1024 }, baseContext);
            expect(result.success).toBe(true);
            const output = JSON.parse(result.output);
    ...
    describe("file_write", () => {
        it("should write new file", async () => {
            const result = await fileWriteTool.execute({ path: "output.txt", content: "Hello, World!" }, baseContext);
            expect(result.success).toBe(true);
        ...
        it("should overwrite existing file", async () => {
            ...
            const result = await fileWriteTool.execute({ path: "existing.txt", content: "new content" }, baseContext);
            expect(result.success).toBe(true);
        ...
        it("should append to file", async () => {
            ...
            const result = await fileWriteTool.execute({ path: "append.txt", content: "line2\n", mode: "append" }, baseContext);
            expect(result.success).toBe(true);
            const content = await fs.readFile(path.join(tempDir, "append.txt"), "utf-8");
            expect(content).toBe("line1\nline2\n");
        });
        it("should create parent directories", async () => {
            const result = await fileWriteTool.execute({ path: "new/nested/dir/file.txt", content: "nested!" }, baseContext);
            expect(result.success).toBe(true);
            const content = await fs.readFile(path.join(tempDir, "new/nested/dir/file.txt"), "utf-8");
            expect(content).toBe("nested!");
        });
        it("should block path traversal", async () => {
            const result = await fileWriteTool.execute({ path: "../outside.txt", content: "malicious" }, baseContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain("越界");
        });
        it("should block sensitive paths", async () => {
            const result = await fileWriteTool.execute({ path: ".env.local", content: "SECRET=hack" }, baseContext);
            expect(result.success).toBe(false);
            expect(result.error).toContain("敏感文件");
        });
        it("should enforce allowedPaths whitelist", async () => {
            ...
            // 不在白名单中
            const result1 = await fileWriteTool.execute({ path: "forbidden/file.txt", content: "test" }, restrictedContext);
            expect(result1.success).toBe(false);
            expect(result1.error).toContain("白名单");
            // 在白名单中
            const result2 = await fileWriteTool.execute({ path: "output/file.txt", content: "allowed" }, restrictedContext);
            expect(result2.success).toBe(true);
        });
        it("should block denied paths", async () => {
            const result = await fileWriteTool.execute({ path: "node_modules/malicious.js", content: "bad code" }, baseContext);
            expect(result.success).toBe(false);
