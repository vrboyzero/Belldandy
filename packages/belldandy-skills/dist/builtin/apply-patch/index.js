...
export const applyPatchTool = {
    definition: {
        name: "apply_patch",
        description: "使用 Unified Diff 变体格式（基于 Blocks）修改一个或多个文件。支持在一次调用中执行添加、删除、更新和移动操作。**这是修改代码的首选方式**。",
        parameters: {
            type: "object",
            properties: {
                input: {
    ...
    async execute(args, context) {
        ...
        try {
            ...
            const recordSummary = (bucket, file) => {
                if (seen[bucket].has(file))
                    return;
            ...
            for (const hunk of parsed.hunks) {
                ...
                if (context.policy.allowedPaths.length > 0) {
                    ...
                    if (!allowed)
                        throw new Error(`[${relative}] 路径不在写入白名单中`);
                }
                if (hunk.kind === "add") {
                ...
                if (hunk.kind === "update") {
                    ...
                    if (hunk.movePath) {
                        ...
                        if (!moveCheck.ok)
                            throw new Error(`[MoveTo: ${hunk.movePath}] ${moveCheck.error}`);
