...
export const terminalTool = {
    ...
    async execute(args, context) {
        ...
        try {
            if (action === "start") {
                const sessionId = await manager.createSession(args.cmd || "", args.args || [], {
                    cwd: args.cwd,
                    cols: args.cols,
                    rows: args.rows,
                });
            ...
            if (!sessionId)
                throw new Error(`Missing 'id' for action '${action}'`);
            if (action === "write") {
                const data = args.data;
                if (data === undefined)
                    throw new Error("Missing 'data' for action 'write'");
            ...
            if (action === "resize") {
                manager.resize(sessionId, args.cols || 80, args.rows || 24);
                return {
                    id,
                    name,
                    success: true,
