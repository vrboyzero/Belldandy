...
const toolGroups = new Set((readEnv("BELLDANDY_TOOL_GROUPS") ?? "all").split(",").map(s => s.trim().toLowerCase()));
const hasToolGroup = (group) => toolGroups.has("all") || toolGroups.has(group);
...
const videoUploadConfig = videoFileApiUrl ? { apiUrl: videoFileApiUrl, apiKey: videoFileApiKey || openaiApiKey || "" } : undefined;
// Model Failover
