...
export function createHookRunner(registry, options = {}) {
     ...
     */
    function runSyncHook(hookName, hooks, event, ctx) {
        if (hooks.length === 0)
            return undefined;
        ...
        for (const hook of hooks) {
            try {
                const out = hook.handler(current, ctx);
                // 检查是否意外返回了 Promise（同步钩子不支持异步）
                if (out && typeof out.then === "function") {
                    const msg = `[hooks] ${hookName} 处理器（来源: ${hook.source}）返回了 Promise；` +
                        `此钩子仅支持同步执行，结果已被忽略。`;
                    if (catchErrors) {
                        logger?.warn?.(msg);
                        continue;
    ...
    function runToolResultPersist(event, ctx) {
        ...
        for (const hook of hooks) {
            try {
                ...
                // 检查是否意外返回了 Promise
                if (out && typeof out.then === "function") {
                    const msg = `[hooks] tool_result_persist 处理器（来源: ${hook.source}）返回了 Promise；` +
                        `此钩子仅支持同步执行，结果已被忽略。`;
                    if (catchErrors) {
                        logger?.warn?.(msg);
                        continue;
