 ...
 * 所有钩子名称的联合类型
 */
export type HookName = "before_agent_start" | "agent_end" | "before_compaction" | "after_compaction" | "message_received" | "message_sending" | "message_sent" | "before_tool_call" | "after_tool_call" | "tool_result_persist" | "session_start" | "session_end" | "gateway_start" | "gateway_stop";
/**
 * Agent 钩子上下文
 */
...
export interface HookHandlerMap {
    before_agent_start: (event: BeforeAgentStartEvent, ctx: HookAgentContext) => Promise<BeforeAgentStartResult | void> | BeforeAgentStartResult | void;
    agent_end: (event: AgentEndEvent, ctx: HookAgentContext) => Promise<void> | void;
    before_compaction: (event: BeforeCompactionEvent, ctx: HookAgentContext) => Promise<void> | void;
    after_compaction: (event: AfterCompactionEvent, ctx: HookAgentContext) => Promise<void> | void;
    message_received: (event: MessageReceivedEvent, ctx: HookMessageContext) => Promise<void> | void;
    message_sending: (event: MessageSendingEvent, ctx: HookMessageContext) => Promise<MessageSendingResult | void> | MessageSendingResult | void;
    message_sent: (event: MessageSentEvent, ctx: HookMessageContext) => Promise<void> | void;
    before_tool_call: (event: BeforeToolCallEvent, ctx: HookToolContext) => Promise<BeforeToolCallResult | void> | BeforeToolCallResult | void;
    after_tool_call: (event: AfterToolCallEvent, ctx: HookToolContext) => Promise<void> | void;
    tool_result_persist: (event: ToolResultPersistEvent, ctx: HookToolResultPersistContext) => ToolResultPersistResult | void;
    session_start: (event: SessionStartEvent, ctx: HookSessionContext) => Promise<void> | void;
    session_end: (event: SessionEndEvent, ctx: HookSessionContext) => Promise<void> | void;
    gateway_start: (event: GatewayStartEvent, ctx: HookGatewayContext) => Promise<void> | void;
    gateway_stop: (event: GatewayStopEvent, ctx: HookGatewayContext) => Promise<void> | void;
}
/**
 * 钩子注册项
 */
...
export interface AgentHooks {
     ...
     */
    beforeRun?: (event: BeforeRunEvent, context: AgentHookContext) => Promise<void | Partial<AgentRunInput>> | void | Partial<AgentRunInput>;
    /**
     * 运行后钩子
     *
     ...
     */
    afterRun?: (event: AfterRunEvent, context: AgentHookContext) => Promise<void> | void;
    /**
     * 工具调用前钩子
     *
     * 在工具执行之前调用。
     ...
     */
    beforeToolCall?: (event: LegacyBeforeToolCallEvent, context: AgentHookContext) => Promise<boolean | JsonObject | void> | boolean | JsonObject | void;
    /**
     * 工具调用后钩子
     *
     * 在工具执行完成后调用。
     ...
     */
    afterToolCall?: (event: LegacyAfterToolCallEvent, context: AgentHookContext) => Promise<void> | void;
}
