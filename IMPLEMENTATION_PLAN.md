...
| P1-4 | 定时任务 / Cron 工具（软硬结合） | §8 定时任务 | 在 Heartbeat Runner 之上，抽象出通用 Cron 工具与配置（例如 `cron.list` / `cron.set`），对接 Gateway 配置，让非心跳类任务也能按计划触发（对标 OpenClaw 的 cron jobs）。 | ✅ **方案 A (轻量 MVP) 已完成 (2026-02-10)** <br> 实现了 `at`（一次性）+ `every`（周期）两种调度，`systemEvent` Payload，JSON 文件持久化（`cron-jobs.json`），Agent `cron` 工具（`list/add/remove/status`），Gateway `BELLDANDY_CRON_ENABLED` 开关。 <br><br> **方案 B 升级路径（未来按需）：** <br> ① 引入 `croner` 库支持 cron 表达式（如 `0 9 * * 1`） <br> ② `main`/`isolated` 会话隔离（避免定时任务污染主会话） <br> ③ `agentTurn` Payload 支持独立 Agent 对话 + 模型/thinking 覆盖 <br> ④ `announce`/`none` + 渠道定向推送（如推送到飞书/Telegram） <br> ⑤ `cron.*` Gateway 协议方法（前端 UI 管理） <br> ⑥ `run`/`runs` action（手动触发 + 执行历史查询） |
