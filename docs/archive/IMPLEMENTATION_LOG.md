...
- **Phase 0**：文档与边界
- **Phase 1 / 1.5 / 2 / 2.5 / 2.6 / 3（UI）**：工程骨架 → 最小闭环 → 极速启动 → 配置 & Doctor → 工具管理 → 极致美学重构
...
### Phase 2.6: 可视化工具管理 (Tool & MCP Management UI) [已完成]

**核心目标**：赋予用户精细控制 Agent 能力边界的权限，允许在运行时动态启用/禁用特定的工具、MCP 服务器或插件。

**已落地 (2026-02-15)**：
- **UI 实现**:
    - WebChat 顶部新增工具设置入口 (🛠️)。
    - 支持 Builtin / MCP / Plugins 分类管理。
- **配置持久化**:
    - 实时开关工具状态，并持久化到 Server 配置。
    - 后端新增 `tools.list` / `tools.update` 协议。
...
| P3-5 | 自身自动更新机制 (Self-Update) | §11 平台与运维 | 实现 `update` 工具或 `gateway update` 命令，自动执行 `git pull` + `pnpm install` + `restart` 流程。 | 需解决 Windows 下的文件占用锁定问题；保持“默认手动，可选自动”的原则。 |
